  ...
  <div class="money-upsell" v-if="show">
    ...
    <b-row>
      <b-col
        ...
        md="6"
        lg="4"
        v-for="(product, index) in upSellData.products"
        :key="index"
      >
        <div class="mb-4 text-center money-upsell-product">
          ...
          <p class="font-weight-bold text-uppercase text-warning mb-2">
            {{ product.price | currencyFormat }}
          </p>
          <b-link
            class="border border-primary d-inline-block rounded px-2 py-1 money-upsell-link"
            :href="product.link"
            target="_blank"
          >
            <span>Mua hàng</span>
...
// URL of upsell file
const upSellUrl = getFileUrl('upsell.json');
