...
// API instances
const { url, coupleApi } = APIS;
const coupleApiUrl = url + coupleApi;
...
export default {
  ...
  watch: {
    firstYear() {
      ...
      if (this.firstYear != null) {
        ...
        this.month = this.firstYear
          .toString()
          .substring(this.firstYear.toString().length - 2);
  ...
  mounted() {
    ...
    this.setYear();
    // Get data
    this.getData(coupleApiUrl, this.parseData);
  },
};
</script>
